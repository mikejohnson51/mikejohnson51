...
\begin{center}
 Spatial Data Representation, Creation and Application; 
 Geoinformatics (GIS);
...
\textbf{March 2021 (Expected)} & \textbf{University of California, Santa Barbara, California (UCSB)} 
\vspace{2mm}
...
\vspace{2mm}
\newline  $>$ \textbf{Dissertation:} Spatial Challenges of $21^{st}$ Century Water Resource Research
\vspace{2mm}
\vspace{2mm}\\
...
\newline
\end{tabular}
*obtained security clearance (secret)
\newline
\vspace{2mm}
...
award_table <- function(awards) {
  ...
  names(awards) <- c("Year","Purpose", "Amount", "Source")
...
c("Author, Creator", "Author", "Contributor") %>% purrr::walk(create_software_sections)
...
\hspace*{0mm}
\begin{tabular}{p{4cm}P{13.5cm}}
\textbf{Summer 2020} & \textbf{Geograhpy 176}: Introduction to Geographic Information Science
\vspace{2mm}
...
teaching <- teaching %>%
  ...
  group_by(class, class_link, type) %>%
